...
This repository builds a scale testing workflow to test new HPC infrastructure. This workflow tests the scaling of the cluster for SW4 in the following ways
...
1. Copy the `flow` directory contents to `~/cylc-src/scale_testing`.
2. Set `SW4` environment variable in the flow.cylc.
3. Adjust cluster settings, core counts, etc in the `flow.cylc`, `strong_scaling.csv`, `weak_scaling.csv` files.
4. `cylc vip scale_testing`.
5. Collect the cylc database at the end of the run to process the results
