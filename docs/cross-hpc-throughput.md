...
| rch (hcpu), no NaN   | 332 / 294–378     | 344 / 325–355     |

Genoa is ~2× faster per core than milan or RCH, and it's also the
*tightest* — measurements vary by ~13 % across configurations. RCH is
similarly tight (~16 % range) but at roughly half genoa's throughput.
...
| rch,   no NaN  | 378 | 294 | **77 %** |
| milan, no NaN  | 479 | 327 | 68 % (dominated by hardware noise) |

Genoa scales very well to 4 nodes. RCH's 77 % is solid, particularly
since the absolute throughput is steady across all four points
(294–378). The milan number is consistent with what its weak-scaling
curve also shows: the partition is too heterogeneous for a single
efficiency number to be meaningful at this sample size.
...
Both genoa and RCH are excellent here. Milan degrades, but with the
caveat that the underlying nodes weren't the same set across the four
runs.
...
**Caveat on the RCH archive**: the tarball was pulled while the two
504-core jobs were still queued on RCH, so its `log/db` only has
the 126 / 252 / 378 timings. The 504-core numbers used here come
from the on-RCH sqlite query
(`strong_test504` ran 56 m 44 s ≈ 3404 s; `weak_test504` ran 3 h
10 m 25 s ≈ 11 425 s). Re-pulling the archive after the workflow
fully completes will let `compare_scaling.py` reproduce the n=4
numbers without manual splicing.
